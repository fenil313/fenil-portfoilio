...
import gsap from 'gsap';
import { ScrollTrigger } from 'gsap/ScrollTrigger';
import { FaReact, FaJs, FaHtml5, FaPython, FaDocker, FaAws, FaGitAlt } from 'react-icons/fa';
...
const Skills = () => {
  ...
  return (
    // Added pt-24 (or more) to account for fixed navbar height
    <main ref={container} className="bg-[#020202] text-white min-h-screen pt-24 pb-16 px-6 md:px-16 lg:px-32 font-mono">
      
      {/* HEADER - Wrapped in a block to prevent layout drift */}
      <header className="mb-20">
       
      </header>

      {/* MATRIX */}
      <section className="matrix-container space-y-16">
        {DATA_POINTS.map((set, idx) => (
          <div key={idx} className="skill-row flex flex-col md:flex-row border-b border-white/5 pb-12">
            <div className="w-full md:w-1/4 text-[10px] tracking-widest text-white/30 mb-6 md:mb-0">
              {set.label}
            </div>
            
            <div className="w-full md:w-3/4 grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-8">
              {set.skills.map((skill, i) => (
                <div key={i} className="group flex flex-col items-center gap-3">
                  <div 
                    ...
                    style={{ color: '#333' }}
                    onMouseOver={(e) => e.currentTarget.style.color = skill.c}
                    onMouseOut={(e) => e.currentTarget.style.color = '#333'}
                  >
                    {skill.i}
                  </div>
                  <span className="text-[9px] font-bold tracking-widest text-[#333] group-hover:text-white transition-colors duration-300">
                    {skill.n}
                  </span>
                </div>
              ))}
